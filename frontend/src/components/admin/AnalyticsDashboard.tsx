  ...
  if (loading) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-6">
        ...
        <p className="text-[12px] font-black text-slate-500 uppercase tracking-widest animate-pulse">Synchronizing Neural Data Stream...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        ...
        <div className="flex gap-4">
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl px-6 py-4 flex items-center gap-3">
             ...
             <span className="text-[12px] font-black text-blue-400 uppercase tracking-widest">{summary.online_now} Nodes Active Now</span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
           <div className="bg-[#0f1133] border border-white/5 rounded-[4rem] p-8 h-full min-h-[450px] relative overflow-hidden shadow-2xl transition-all hover:border-blue-500/20">
             <div className="flex justify-between items-center mb-12">
               ...
               <div className="flex gap-4">
                 <div className="flex items-center gap-3"><div className="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_10px_blue]"></div><span className="text-[12px] font-black uppercase text-slate-500 tracking-widest">System Events</span></div>
               </div>
             </div>
                ...
                {[45, 60, 40, 80, 55, 95, 70, 85, 40, 60, 90, 75, 50, 65].map((h, i) => (
                   <div key={i} className="flex-1 flex flex-col items-center gap-4 group relative">
                     <div 
                       ...
                       style={{ height: `${h}%` }}
                     ></div>
                     <span className="text-[12px] font-black text-slate-700 uppercase tracking-widest leading-none mt-2">{8 + i}:00</span>
                   </div>
                ))}
             </div>
             <p className="text-[12px] text-slate-500 font-black uppercase tracking-[0.2em] mt-8 italic text-center text-balance">Activity pulse generated from real-time database audit streams and interaction vectors.</p>
           </div>
        </div>
              ...
              <div className="space-y-10">
                 {locations.length > 0 ? locations.map((loc: any, i: number) => (
                   ...
                   <div className="h-full flex flex-col items-center justify-center space-y-4 py-20 opacity-30">
                     ...
                     <p className="text-[12px] font-black uppercase tracking-widest">No Geo Data Recorded</p>
                   </div>
                 )}
              </div>
           </div>
            ...
            <div className="space-y-6">
               {(stats?.user_distribution || []).map((p: any, i: number) => (
                 <div key={i} className="flex justify-between items-center group cursor-pointer hover:bg-white/5 p-4 rounded-2xl transition-all">
                    <span className="text-[12px] font-black text-slate-400 uppercase tracking-widest group-hover:text-white transition-colors">ROLE: {p.role}</span>
                    <span className="text-[12px] font-black text-white italic">{p.count} Nodes</span>
                 </div>
               ))}
               {(!stats?.user_distribution || stats.user_distribution.length === 0) && (
                 <p className="text-[12px] text-slate-500 uppercase font-black text-center py-10">Awaiting Segmentation Data...</p>
               )}
            </div>
         </div>
            ...
            <div className="space-y-6">
               {[
                 ...
                 <div key={i} className="flex items-center gap-6">
                    <div className={`text-[12px] font-black italic uppercase w-40 ${s.color} truncate tracking-widest`}>{s.source}</div>
                    <div className="flex-1 h-2 bg-white/5 rounded-full overflow-hidden">
                       <div className={`h-full bg-current shadow-[0_0_10px_currentColor]`} style={{ width: `${s.val}%` }}></div>
                    </div>
                    <span className="text-[12px] font-black text-white w-10 text-right">{s.val}%</span>
                 </div>
               ))}
            </div>
         </div>

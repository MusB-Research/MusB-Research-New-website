        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div>
                    ...
                    <p className="text-[12px] text-slate-500 font-bold uppercase tracking-[0.3em] mt-2 italic">Clinical Study Reports & Regulatory Approval Queue</p>
                </div>
            ...
            <div className="bg-indigo-600/10 border border-indigo-500/20 rounded-[2.5rem] p-10 flex flex-col md:flex-row md:items-center justify-between gap-8 relative overflow-hidden">
                ...
                <div className="space-y-4 relative z-10">
                    <div className="flex items-center gap-3 bg-indigo-500/20 w-fit px-4 py-1.5 rounded-full border border-indigo-500/30">
                        ...
                        <span className="text-[12px] font-black text-indigo-400 uppercase tracking-widest underline decoration-2 underline-offset-4">Regulated Environment (21 CFR Part 11)</span>
                    </div>
                    ...
                    <p className="text-[12px] text-slate-400 font-bold uppercase tracking-widest max-w-xl">You have <span className="text-white">04 Reports</span> awaiting your clinical verification. These must be reviewed and signed within 24 hours to maintain regulatory compliance.</p>
                </div>
                <button className="px-10 py-5 bg-white text-slate-950 rounded-2xl text-[12px] font-black uppercase tracking-[0.2em] shadow-2xl hover:scale-[1.05] transition-all shrink-0">Review Sign-Off Queue</button>
            </div>
                    ...
                    >
                        <div className="flex items-start justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-[12px] text-indigo-400 font-black uppercase tracking-widest italic">{r.type}</p>
                                    <h4 className="text-xl font-black text-white italic truncate tracking-tight">{r.title}</h4>
                                </div>
                            </div>
                            <div className={`px-4 py-1.5 rounded-xl border text-[12px] font-black uppercase tracking-widest ${
                        ...
                        <p className="text-[12px] text-slate-500 font-bold italic leading-relaxed uppercase tracking-tight line-clamp-2">{r.description}</p>

                        <div className="pt-8 border-t border-white/5 flex items-center justify-between">
                             <div>
                                <p className="text-[12px] text-slate-700 font-black uppercase tracking-widest">Authored By</p>
                                <p className="text-[12px] text-slate-300 font-black uppercase tracking-widest mt-1">{r.author} • {r.date}</p>
                            </div>
                            {r.status === 'Pending' && (
                                 <div className="flex gap-3">
                                    <button onClick={() => handleSignOff(r)} className="px-6 py-3 bg-indigo-600 text-white rounded-xl text-[12px] font-black uppercase tracking-widest flex items-center gap-2 hover:scale-[1.03] transition-all shadow-xl shadow-indigo-900/40">
                                        Push to Participant <CheckCircle2 className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleSignOff(r)} className="px-6 py-3 bg-white/5 border border-white/10 text-indigo-400 rounded-xl text-[12px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-white/10 transition-all">
                                        Apply Sign-Off <FileSignature className="w-4 h-4" />
                                    </button>
                                </div>
                            )}
                            {r.status === 'Signed' && (
                                 <button className="px-6 py-3 bg-white/5 border border-white/5 text-emerald-400 rounded-xl text-[12px] font-black uppercase tracking-widest flex items-center gap-2">
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0B101B]/95 backdrop-blur-xl p-6">
                        ...
                        >
                            ...
                            <div>
                                ...
                                <p className="text-[12px] text-slate-500 font-bold uppercase tracking-widest mt-2">{selectedReport?.id} • Identity Confirmation Required</p>
                            </div>
                            <div className="space-y-6">
                                <p className="text-sm text-slate-400 font-bold italic">By signing this document, I confirm that the data represented is accurate to the best of my medical knowledge and complies with the clinical protocol.</p>
                                 <div className="space-y-3">
                                    <label className="text-[12px] font-black text-slate-600 uppercase tracking-widest">Enter Verification Code</label>
                                    <input 
                                        type="password" 
                                        placeholder="••••••" 
                                        value={twoFactorCode}
                                    ...
                                    />
                                    <p className="text-[12px] text-slate-700 font-black uppercase tracking-widest">Code sent to registered mobile device</p>
                                </div>
                            </div>
                             <div className="flex gap-4">
                                 <button onClick={() => setIsSigning(false)} className="flex-1 py-4 bg-white/5 border border-white/5 text-slate-400 rounded-2xl text-[12px] font-black uppercase tracking-widest">Cancel</button>
                                 <button onClick={confirmSignOff} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl text-[12px] font-black uppercase tracking-widest shadow-xl shadow-indigo-600/30">Verify & Apply Stamp</button>
                             </div>
                        </motion.div>
                    </div>
                )}

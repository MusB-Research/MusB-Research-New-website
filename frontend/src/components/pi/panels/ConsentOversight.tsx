...
export default function ConsentOversight() {
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            case 'Signed': return 'text-teal-400 bg-teal-500/10 border-teal-500/20';
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-10">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div>
                    <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">Consent <span className="text-teal-400">Oversight</span></h2>
                    <p className="text-base text-slate-500 font-bold uppercase tracking-[0.3em] mt-2 italic">Legal Compliance & ICF Tracking</p>
                </div>
                <div className="flex items-center gap-6">
                    <div className="flex items-center gap-3 px-6 py-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                        ...
                        <div>
                            <p className="text-sm text-slate-500 font-bold uppercase">Compliance</p>
                    ...
                    <div className="flex items-center gap-3 px-6 py-4 bg-red-500/5 border border-red-500/10 rounded-2xl">
                        ...
                        <div>
                            <p className="text-sm text-slate-500 font-bold uppercase">Actions Req.</p>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0B101B]/40 border border-white/5 p-4 rounded-3xl">
                    ...
                    {['All', 'Pending', 'Expired', 'Action Required'].map((f: any) => (
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-xl text-sm font-black uppercase tracking-widest transition-all ${
                                activeFilter === f ? 'bg-teal-600 text-white' : 'text-slate-500 hover:text-white'
                            }`}
                        >
                            {f}
                        </button>
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="bg-white/5 border border-white/10 rounded-xl pl-10 pr-6 py-2.5 text-sm text-white font-bold outline-none focus:border-teal-500/50 transition-all w-60 uppercase tracking-widest placeholder:text-slate-700"
                    />
                </div>
            </div>
                ...
                <AnimatePresence mode="popLayout">
                    {filteredRecords.map((r) => (
                        <motion.div 
                            ...
                            className="bg-[#0B101B]/40 border border-white/5 rounded-[2rem] p-8 space-y-6 hover:border-teal-500/30 transition-all group relative overflow-hidden"
                        >
                            <div className="absolute top-0 right-0 p-6 opacity-5 group-hover:rotate-12 group-hover:opacity-20 transition-all">
                                <ScrollText className="w-12 h-12 text-teal-400" />
                            </div>

                            <div className="flex items-center justify-between relative z-10">
                                <div className={`px-3 py-1 rounded-lg border text-sm font-black uppercase tracking-tighter ${getStatusStyle(r.status)} shadow-[0_0_10px_rgba(var(--status-rgb),0.1)]`}>
                                    {r.status}
                                </div>
                                <p className="text-sm text-slate-700 font-mono tracking-widest">{r.signedDate}</p>
                            </div>

                            <div className="space-y-1">
                                <p className="text-sm text-teal-400 font-black uppercase tracking-[0.2em] italic underline decoration-indigo-800 underline-offset-4">{r.type}</p>
                                <h4 className="text-xl font-black text-white italic truncate">{r.subjectName}</h4>
                                <p className="text-sm text-slate-500 font-bold uppercase tracking-widest">{r.subjectId} • {r.method}</p>
                            </div>

                            <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                                <div>
                                    <p className="text-sm text-slate-600 font-black uppercase">Active Version</p>
                                    <p className="text-sm text-white font-black uppercase italic tracking-widest mt-0.5">{r.version}</p>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <button className="p-3 bg-teal-600 text-white rounded-xl hover:scale-105 active:scale-95 transition-all shadow-lg shadow-teal-600/20"><ChevronRight className="w-4 h-4" /></button>
                                </div>
                            </div>
                        </motion.div>
                    ))}

...
export default function AuditLogModule() {
    ...
    const entries: AuditEntry[] = [
        ...
        { id: 'LOG-884', action: 'Study Manual Updated', category: 'System', user: 'Admin Console', role: 'System', timestamp: '2026-03-20 16:00', details: 'Study v3.4 push to production complete.', status: 'Verified' },
    ...
    const getCategoryStyle = (category: string) => {
        switch (category) {
            ...
            case 'System': return 'text-teal-400 bg-teal-500/10 border-teal-500/20';
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                <div>
                    <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">Immutable <span className="text-teal-400">Audit Log</span></h2>
                    <p className="text-base text-slate-500 font-bold uppercase tracking-[0.3em] mt-2 italic">21 CFR Part 11 Compliant Digital Ledger</p>
                </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[#0B101B]/60 border border-white/5 p-4 rounded-3xl">
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="bg-white/5 border border-white/10 rounded-2xl pl-12 pr-6 py-3 text-sm text-white font-bold outline-none focus:border-teal-500/50 transition-all w-72 uppercase tracking-widest font-mono placeholder:text-slate-700"
                    />
                </div>
            </div>

            {/* Audit Feed */}
            <div className="bg-[#0B101B]/40 border border-white/5 rounded-[3rem] p-2 lg:p-4 space-y-2">
                {filteredEntries.map((log) => (
                    <motion.div 
                        key={log.id} 
                        layout 
                        className="p-4 rounded-[2rem] hover:bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all grid grid-cols-1 lg:grid-cols-4 gap-8 group"
                    >
                        <div className="lg:col-span-1 flex items-center gap-6">
                            <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center text-slate-700 group-hover:bg-teal-600/10 group-hover:text-teal-400 group-hover:border-teal-500/20 transition-all border border-transparent">
                                <HistoryIcon className="w-6 h-6" />
                            </div>
                            <div>
                                <p className="text-sm text-slate-500 font-black uppercase tracking-widest italic">{log.timestamp}</p>
                                <div className={`inline-flex px-2 py-0.5 rounded-md text-sm font-black uppercase tracking-widest mt-1 border ${getCategoryStyle(log.category)}`}>
                                    {log.category}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-2">
                            ...
                            <p className="text-sm text-slate-500 font-bold uppercase tracking-tight italic leading-relaxed">{log.details}</p>
                        </div>

                        <div className="lg:col-span-1 flex items-center justify-end gap-10">
                            <div className="text-right">
                                <p className="text-base text-white font-black uppercase tracking-widest">{log.user}</p>
                                <p className="text-sm text-slate-600 font-black uppercase tracking-widest">{log.role}</p>
                            </div>
            ...
            <div className="p-5 bg-emerald-500/5 border border-emerald-500/10 rounded-[3rem] flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-base text-slate-500 font-bold uppercase tracking-widest mt-1">Hash Verification: SHA-256 Validated • Blockchain Anchor Active</p>
                    </div>
                </div>
                <div className="flex gap-4">
                    <button className="px-8 py-4 bg-white/5 border border-white/10 text-slate-400 rounded-2xl text-sm font-black uppercase tracking-widest hover:text-white transition-all">Verify Chain</button>
                </div>
            </div>
        </motion.div>
    );

...
export default function PIVisitsAssessmentsModule() {
    ...
    const handleScheduleVisit = () => {
        ...
        const newVisit: Visit = {
            id: 'v' + Date.now(),
            name: schedulePanelForm.visitType,
            type: schedulePanelForm.visitType,
            targetDate: schedulePanelForm.date,
            windowDays: 3,
            scheduledDate: schedulePanelForm.date,
            actualDate: null,
            status: 'Scheduled',
            location: schedulePanelForm.location as any,
            checklist: [{ label: 'Consent verified', checked: false }, { label: 'Vitals collected', checked: false }],
            assessments: [],
            vitals: { weight: '', height: '', bp: '', hr: '', temp: '' },
            aes: [],
            notes: schedulePanelForm.notes ? [{ author: 'PI (You)', role: 'PI', time: new Date().toLocaleTimeString(), text: schedulePanelForm.notes }] : [],
            documents: [],
            deviations: [],
            samples: [],
            dispensing: [],
        ...
        setParticipants(prev => prev.map(p => p.id === schedulePanelForm.participantId ? {
            ...p,
            visits: [...p.visits, newVisit]
        } : p));

        setSchedulePanel(false);
        setActiveParticipantId(schedulePanelForm.participantId);
        setActiveVisitId(newVisit.id);
        addToast('Visit Scheduled Successfully');
        logAudit(`Visit ${newVisit.name} scheduled for ${schedulePanelForm.participantId}`);
    };

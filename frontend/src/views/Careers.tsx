...
export default function Careers() {
    ...
    return (
        <div className="min-h-screen font-sans text-slate-200 relative overflow-x-hidden">
            <SEO 
                title="Careers in Clinical Research | Join MUSB Health"
                description="Join the future of medical science. Explore career opportunities in clinical trial coordination, research, and laboratory operations at MUSB Health."
                canonical="https://www.musbhealth.com/careers"
            />
            ...
            < section id="why-musb" className="max-w-[1700px] mx-auto px-6 md:px-12 py-20 border-t border-white/5" >
                <div className="grid lg:grid-cols-3 gap-16 items-start">
                    <div className="space-y-8 lg:sticky lg:top-32">
                        <span className="text-cyan-400 font-black text-[12px] uppercase tracking-[0.4em]">Excellence & Growth</span>
                            ...
                            <div className="space-y-10">
                                ...
                                <div className="flex gap-12 text-center border-t border-white/10 pt-10">
                                    <div>
                                        ...
                                        <div className="text-[12px] font-black uppercase tracking-widest text-slate-500 mt-2">Data Integrity</div>
                                    </div>
                                    <div>
                                        <div className="text-3xl font-black text-white">15+</div>
                                        <div className="text-[12px] font-black uppercase tracking-widest text-slate-500 mt-2">Yrs Experience</div>
                                    </div>
                                    <div>
                                        <div className="text-3xl font-black text-white">FL-TN</div>
                                        <div className="text-[12px] font-black uppercase tracking-widest text-slate-500 mt-2">Regional Reach</div>
                                    </div>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                {cultureValues.map((val) => (
                                    <div key={val.name} className="bg-white/5 border border-white/10 p-8 rounded-3xl flex flex-col items-center justify-center text-center space-y-4 hover:border-cyan-500 transition-all group">
                                        ...
                                        <div className="text-[12px] font-black uppercase tracking-widest text-white">{val.name}</div>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            < section className="max-w-[1700px] mx-auto px-6 md:px-12 py-20" >
                <div className="text-center space-y-6 mb-16">
                    <span className="text-cyan-400 font-black text-[12px] uppercase tracking-[0.4em]">Diverse Opportunities</span>
                        ...
                        return (
                            <div key={cat.id} className="group bg-white/5 border border-white/5 p-8 md:p-12 rounded-[3.5rem] hover:bg-slate-900 transition-all duration-500 shadow-xl overflow-hidden relative">
                                ...
                                <div className="relative z-10 space-y-6">
                                    ...
                                    <a href="#open-positions" className="inline-flex items-center gap-2 text-[12px] font-black uppercase tracking-widest text-white border-b-2 border-cyan-500 pb-1 hover:gap-3 transition-all pt-2 group-hover:text-cyan-400">
            ...
            < section id="open-positions" className="bg-slate-900/50 py-20 scroll-mt-24" >
                <div className="max-w-[1400px] mx-auto px-6 md:px-12">
                    <div className="flex flex-col md:flex-row items-end justify-between gap-8 mb-16">
                        <div className="space-y-6">
                            <span className="text-cyan-400 font-black text-[12px] uppercase tracking-[0.4em]">Current Vacancies</span>
                        ...
                        <div className="flex bg-white/5 p-1.5 rounded-2xl border border-white/10 flex-wrap">
                            {departments.map(dept => (
                                <button
                                    ...
                                    className={`px-6 py-3 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all ${activeDept === dept ? 'bg-cyan-500 text-slate-950 shadow-lg' : 'text-slate-500 hover:text-white'
                                        }`}
                                >
                                    {dept}
                                </button>
                    ...
                    <div className="grid gap-6">
                        {loading ? (
                            <div className="py-24 flex flex-col items-center justify-center space-y-4">
                                ...
                                <p className="text-slate-500 font-bold uppercase tracking-widest text-[12px] animate-pulse">Syncing Openings...</p>
                            </div>
                                ...
                                <div key={job.id} className="group bg-white/5 border border-white/5 rounded-[2.5rem] p-6 md:p-10 flex flex-col lg:flex-row items-start lg:items-center gap-10 hover:bg-white/10 hover:border-cyan-500/30 transition-all shadow-xl">
                                    <div className="flex-grow space-y-4">
                                        <div className="flex flex-wrap gap-4 items-center">
                                            {job.isFeatured && (
                                                <span className="px-3 py-1 rounded-lg bg-cyan-500 text-slate-950 text-[12px] font-black uppercase tracking-widest">Featured</span>
                                            )}
                                            <span className="text-[12px] font-black uppercase tracking-widest text-slate-500">{job.department}</span>
                                        </div>
                                        <h3 className="text-2xl font-black text-white uppercase group-hover:text-cyan-400 transition-colors italic">{job.title}</h3>
                                        <div className="flex flex-wrap gap-6 text-[12px] font-bold text-slate-400 uppercase tracking-widest">
                                    ...
                                    <Link
                                        to={`/careers/${job.id}`}
                                        className="w-full lg:w-auto px-10 py-5 rounded-2xl bg-white text-slate-950 font-black uppercase tracking-widest text-[12px] hover:bg-cyan-500 transition-all text-center shadow-xl shadow-white/5 whitespace-nowrap"
                                    >
                                        Details & Apply
                                    </Link>
                                </div>
                    ...
                    {hiringProcess.map((step, i) => (
                        <div key={step.id} className="relative z-10 flex flex-col items-center text-center space-y-8 group">
                            <div className="w-24 h-24 rounded-[2rem] bg-slate-900 border border-white/10 flex items-center justify-center text-cyan-400 shadow-2xl group-hover:bg-cyan-500 group-hover:text-slate-950 transition-all duration-500">
                                ...
                                <div className="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-slate-950 border-2 border-cyan-500 flex items-center justify-center text-[12px] font-black text-white group-hover:bg-white group-hover:text-slate-950 transition-colors">{i + 1}</div>
                            </div>
                            <div className="space-y-4">
            ...
            <section className="max-w-4xl mx-auto px-6 py-10 text-center">
                <p className="text-[12px] font-black uppercase tracking-[0.3em] text-slate-300 leading-relaxed">

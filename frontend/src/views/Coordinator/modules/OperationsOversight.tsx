            ...
            <div className={`grid ${isMobile ? 'grid-cols-1' : isTablet ? 'grid-cols-2' : 'grid-cols-4'} gap-6`}>
                {isLoading ? (
                    Array.from({ length: 4 }).map((_, i) => (
                        <div key={i} className="bg-[#1E293B]/40 border border-white/5 p-8 rounded-[2.5rem] animate-pulse">
                    ...
                    <div
                        ...
                        className="bg-[#1E293B]/30 backdrop-blur-xl border border-white/5 p-8 rounded-[2.5rem] hover:bg-white/[0.04] hover:border-blue-500/30 cursor-pointer transition-all group relative overflow-hidden shadow-2xl"
                    >
                        ...
                        <div className="flex items-center gap-4 mb-6 relative z-10">
                            ...
                            <span className="text-[11px] font-black text-slate-500 tracking-[0.2em] uppercase italic">{stat.label}</span>
                        </div>
                        <p className="text-4xl font-black text-white italic tracking-tighter relative z-10">{stat.val}</p>
                    </div>
                ))}
            </div>
                ...
                <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-3'} gap-6`}>
                    {isLoading ? (
                        Array.from({ length: 3 }).map((_, i) => (
                            <div key={i} className="flex items-center justify-between p-8 bg-[#1E293B]/20 border border-white/5 rounded-[2rem]">
                        ...
                        <div key={i} className="flex items-center justify-between p-8 bg-[#1E293B]/20 backdrop-blur-xl border border-white/5 rounded-[2.5rem] hover:bg-white/[0.04] transition-all cursor-pointer group active:scale-[0.98] shadow-2xl">
                            <div className="flex items-center gap-6">
                                <div className={`p-4 rounded-2xl ${item.bg} border border-white/5 shadow-xl`}>
                                    <item.icon className={`w-6 h-6 ${item.color}`} />
                                </div>
                                <div className="flex flex-col gap-1">
                                    <p className="text-[13px] font-black text-white group-hover:text-blue-400 transition-colors uppercase italic tracking-tight">{item.label}</p>
